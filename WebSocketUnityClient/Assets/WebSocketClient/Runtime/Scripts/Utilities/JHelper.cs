...
namespace WebSocketClient
{
    public class JHelper
    {
        ...
        private static JToken GetJsonByType(JToken jObject, string key, JTokenType type)
        {
            ...
            {
                ((JObject)jObject).TryGetValue(key, out token);
            }
        ...
        /// <returns></returns>
        public static bool TryGetDictionaryByStr<T1, T2>(JToken jObject, string key, out Dictionary<T1, T2> value)
        {
            value = null;
            string dicStr = JHelper.GetJsonString(jObject, key);
            if (string.IsNullOrEmpty(dicStr))
            {
                return false;
            }
            value = JsonConvert.DeserializeObject<Dictionary<T1, T2>>(dicStr);
            return value != null;
        }
